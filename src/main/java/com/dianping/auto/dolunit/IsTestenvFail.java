...
*/
public class IsTestenvFail<T> extends DiagnosingMatcher<T> {
    private DolTestCaseParser tcParser;
    private HqlExecutor testHqlExecutor;

    public IsTestenvFail(DolTestCaseParser tcParser, HqlExecutor testHqlExecutor) {
        ...
        if (Description.NONE == mismatch) {
            try {
                ...
                if (0 == testHqlExecutor.isSuccess) {
                    match = StringUtils.contains(testHqlExecutor.result, "fail");
                }
            } catch (Exception e) {
                e.printStackTrace();
        }
    ...
    public void describeTo(Description description) {
        description.appendText("fail");
    }

    @Factory
    public static <T> Matcher<T> isTestenvFail(DolTestCaseParser tcParser, HqlExecutor testHqlExecutor) {
        return new IsTestenvFail<T>(tcParser, testHqlExecutor);
    }
}
